...
class ADTEvaluator(Evaluator):
    def __init__(
        ...
        super(ADTEvaluator, self).__init__(
            pickle_source_path,
            set_subfolder,
            hvo_pickle_filename,
            list_of_filter_dicts_for_subsets,
        ...
        # gt subset evaluator
        self.gt_SubSet_Evaluator = HVOSeq_SubSet_ADTEvaluator(
    ...
    def set_pred(self, model):
        ...
        self.prediction_SubSet_Evaluator = HVOSeq_SubSet_ADTEvaluator(
            self._prediction_subsets,
            self._prediction_tags,
    ...
    def save_as_pickle(self, save_evaluator_path):
        ...
        save_evaluator_path = os.path.join(
            save_evaluator_path, "ADTEvaluator_" + self.__version___
        )
        ...
        filename = os.path.join(
            save_evaluator_path,
            "{}_{}_{}_evaluator.pickle".format(
                self.dataset.dataset_name, self.dataset.split, self.dataset.__version__
            ),
        )
        save_to_pickle(self, filename)


class HVOSeq_SubSet_ADTEvaluator(HVOSeq_SubSet_Evaluator):
    def __init__(
        self,
        set_subsets,
        set_tags,
        ...
        super(HVOSeq_SubSet_ADTEvaluator, self).__init__(
            set_subsets,
            set_tags,
            set_identifier,
    ...
    def get_logging_dict(
        ...
        logging_dict = super(HVOSeq_SubSet_ADTEvaluator, self).get_logging_dict(
...
def log_eval(evaluator, model, log_media, epoch, dump):
    ...
    if log_media:
        ...
        if dump:
            ...
            evaluator.dump(
                "results/evaluator_{}_run_{}_Epoch_{}.Eval".format(
                    evaluator._identifier, wandb.run.name, epoch
                )
            )
